...
> **Status:** The interface is stable and integrated into `DefaultSecurityPolicy` (graceful no-op when no provider is configured) and the x402 payment tool (placeholder signatures). No concrete implementation ships with ch4p yet — a `@ch4p/plugin-erc8004` package is planned.
