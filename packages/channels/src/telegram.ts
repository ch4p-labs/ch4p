...
export class TelegramChannel implements IChannel {
  ...
  async start(config: ChannelConfig): Promise<void> {
    ...
    // NOTE: allowedUsers is expected to contain Telegram user IDs as decimal strings,
    //       not usernames. Telegram identifies users by numeric ID, and we compare
    //       these string IDs directly against incoming update.from.id values.
